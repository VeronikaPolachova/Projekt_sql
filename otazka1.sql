...
SELECT *,
	CASE 
		...
		CASE
			WHEN AVG(salary_interannual_growth) OVER (PARTITION BY industry_branch_name) > 0 THEN 'overall_increase'
			WHEN AVG(salary_interannual_growth) OVER (PARTITION BY industry_branch_name) < 0 THEN 'overall_decrease'
			WHEN AVG(salary_interannual_growth) OVER (PARTITION BY industry_branch_name) = 0 THEN 'overall_stagnation'
			ELSE NULL
...
-- konkretneji
WITH cte_overall_salary_statistics AS (
	WITH cte_salary_history AS (
		SELECT 
			payroll_year,
			industry_branch_name,
...
SELECT
	...
	flag_overall_growth_per_industry
FROM cte_overall_salary_statistics
WHERE 1=1
...
-- AND flag_overall_growth_per_industry = 'overall_decrease' -- pokud chci videt, v jakych odvetvich doslo k celkovemu poklesu mezd za dane casove obdobi
;		
